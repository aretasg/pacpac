...
from .patchdock_tools import (
    output_patchdock_ab_constraint,
    output_patchdock_ag_constraint,
...
def capri_evaluate_test_structures(summary_file, folder="results"):
    ...
    for _, ab_h_chain, ab_l_chain, ag_chain, _, pdb_name in chains:
        ...
        q = process_transformations(
            trans_file, ag_chain, ab_h_chain, ab_l_chain, limit=200
        )
        if q is not None:
            num_decoys[q] += 1
...
def kfold_cv_eval(
    ...
    for i, (train_ids, test_ids) in enumerate(kf.split(np.arange(num_structures))):
        ...
        model.fit(
            [cdrs_train, np.squeeze(mask_train)],
            lbls_train,
            batch_size=32,
            epochs=18,
...
def compute_classifier_metrics(labels, probs):
    ...
    print(
        "Youden's J statistic = {} +/- {}. Using it as threshold.".format(
            jstat, jstat_err
        )
    )
...
def open_crossval_results(
    folder="runs/cv-ab-seq", num_results=10, loop_filter=None, flatten_by_lengths=True
):
    class_probabilities = []
    labels = []
...
def binding_profile(summary_file, probs, threshold=0.5):
    ...
    for i, (_, ab_h_chain, ab_l_chain, _, ab_seq, pdb) in enumerate(
        load_chains(summary_file)
    ):
...
def neighbourhood_tops(weights, top_k=10, num_filters_first=20):
    ...
    for f in range(num_filters_first):
        ...
        idxs = act.argsort()[-top_k:][
            ::-1
        ]  # get indices for top k elements in descending order
...
def evaluate_individual_ab(max_cdr_len, ag_search, ab_h_chain, ab_l_chain, seqs, pdb):
    ...
    cdrs, lbls, masks, (num_in_contact, num_residues) = process_chains(
        ag_search, ab_h_chain, ab_l_chain, seqs, pdb, max_cdr_len
    )

    print(
        "PDB {}, residues in contact: {}, overall: {}".format(
            pdb, num_in_contact, num_residues
        )
    )
