...
def _write_pd_constraint(f, chain_id, res):
    ...
    res_num = str(res.id[1]) + (res.id[2] if res.id[2] != " " else "")
    f.write(res_num + " " + chain_id + "\n")


def output_patchdock_ab_constraint(
    structure, filename="ab-patchdock.txt", cutoff=67.00
):
    model = structure[0]
...
def output_patchdock_ag_constraint(ext_epitope, chain_id, filename="ag-patchdock.txt"):
    with open(filename, "w") as f:
...
def decoy_class(f_nat, l_rmsd, i_rmsd):
    if f_nat >= 0.5 and l_rmsd <= 1.0 or i_rmsd <= 1.0:
        return "high"
    if ((f_nat >= 0.3 and f_nat < 0.5) and (l_rmsd < 5.0 or i_rmsd < 2.0)) or (
        f_nat >= 0.5 and l_rmsd > 1.0 and i_rmsd > 1.0
    ):
        return "med"
    if ((f_nat >= 0.1 and f_nat < 0.3) and (l_rmsd < 10.0 or i_rmsd < 4.0)) or (
        f_nat >= 0.3 and l_rmsd > 5.0 and i_rmsd > 2.0
    ):
        return "low"
    return None


def process_transformations(trans_file, ag_chain, ab_h_chain, ab_l_chain, limit=200):
    f = open(trans_file, "r")
    lines = f.readlines()
    f.close()
    ...
    if num_decoys["high"] > 0:
        return "high"
    elif num_decoys["med"] > 0:
        return "med"
    elif num_decoys["low"] > 0:
        return "low"
    else:
        return None
