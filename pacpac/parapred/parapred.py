...
def process_single_pdb(pdb_file, ab_h_chain_id, ab_l_chain_id):
    ...
    structure = produce_annotated_ab_structure(
        ab_h_chain, ab_l_chain, {"H": None, "L": None}, np.expand_dims(probs, -1)
    )
...
def process_multiple_pdbs(pdb_descr_file, pdb_folder):
    ...
    if any(name not in columns for name in ["pdb", "Hchain", "Lchain"]):
        print(
            "ERROR: The PDB description file must a CSV file with columns 'pdb', "
...
def process_full_VH_VL_sequence(full_sequence):
    cdrs = get_CDR_simple(
        full_sequence.replace("'", "").replace('"', "").replace(" ", "").upper()
    )
    print(cdrs)
