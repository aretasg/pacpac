...
def download_annotated_seq(pdb, hchain, lchain):
    ...
    page = requests.get(
        "http://opig.stats.ox.ac.uk/webapps/sabdab-sabpred/StrViewer.php?str=" + pdb
    )
    tree = html.fromstring(page.content)

    # Get FV info section for the correct pair of chains
    fv_info = tree.xpath("//section[@id='fv_info']")
    chains_info_div_id = fv_info[0].xpath(
        ".//div[@class='accordion-heading']/a[contains(text(), '{} / {}')]/@href".format(
            hchain, lchain
        )
    )[0]
    ...
    ab_seqs_div_id = chains_info[0].xpath(
        ".//div[@class='accordion-heading']/a[contains(text(), "
        "'Chothia-numbered antibody sequences')]/@href"
    )[0]
