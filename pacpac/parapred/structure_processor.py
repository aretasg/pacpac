...
# Config constants
NUM_EXTRA_RESIDUES = (
    2  # The number of extra residues to include on the either side of a CDR
)
...
def residue_seq_to_one(seq):
    three_to_one = (
        lambda r: Polypeptide.three_to_one(r.resname)
        if r.resname in Polypeptide.standard_aa_names
        else "X"
    )
...
def extended_epitope(ag_chain, ab_h_chain, ab_l_chain, cutoff=10.0):
    ...
    ext_epi = filter(
        lambda r: residue_in_contact_with(r, epi_search, dist=cutoff), ag_chain
    )
    return list(ext_epi)
