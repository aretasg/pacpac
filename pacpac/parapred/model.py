...
def ab_ag_seq_model(max_ag_len, max_cdr_len):
    ...
    enc_ag = Bidirectional(
        LSTM(128, dropout=0.1, recurrent_dropout=0.1), merge_mode="concat"
    )(ag_seq)
    ...
    glb_fts = Bidirectional(
        LSTM(256, dropout=0.15, recurrent_dropout=0.2, return_sequences=True),
        merge_mode="concat",
    )(loc_fts)
...
def base_ab_seq_model(max_cdr_len):
    ...
    loc_fts = MaskedConvolution1D(
        28, 3, padding="same", activation="elu", kernel_regularizer=l2(0.01)
    )(seq)
