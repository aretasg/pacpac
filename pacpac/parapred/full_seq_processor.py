...
def get_CDR_simple(
    ...
    for num_tuple, res in res_num_all[0]:
        if num_tuple[0] in cdr1_scheme:
            cdr1 += res  # num_tuple[1] may be an insertion code, (e.g. 111B)
...
def read_fasta(
    ...
    description_parser_function=None,
    use_seq_class=True,
    check_sequence=amino_list1,
    name_first_spilt=True,
):
    """
    ...
    for line in open(filename):
        if line[0] == ">":
            ...
            if use_seq_class:
                sequences += [
                    FakeSeq(
                        seq="", seq_id=seqid, seq_name=name, description=description
                    )
                ]
            else:
                sequences += [""]
            if (
                "|" in name
            ):  # for uniprot downloaded regions (e.g. without signal peptides) the last part may be the amino acid range
    ...
    if remove != []:  # remove sequences not containing only standard amino acids
        ...
        for j in sorted(remove, reverse=True):
            sys.stderr.write(
                "**** SKIPPING sequence %d %s  --> contains NOT STANDARD residues that cannot be processed\n"
                % (j + 1, names[j])
            )
            sys.stderr.flush()
