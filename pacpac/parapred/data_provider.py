...
try:
    ...
    def download_annotated_seq(*args, **kwargs):
        raise Exception(
            "download_annotated_seq is not available because scrape module was not correctly imported"
...
def load_chains(
    dataset_desc_filename, sequence_cache_file="parapred/precomputed/downloaded_seqs.p"
):
    ...
    for _, entry in df.iterrows():
        ...
        if "|" in ag_chain:  # Several chains
            ...
            ag_atoms = [
                a for c in chain_ids for a in Selection.unfold_entities(model[c], "A")
            ]
        else:  # 1 chain
        ...
        yield ag_search, model[ab_h_chain], model[
            ab_l_chain
...
def process_dataset(summary_file):
    ...
    for ag_search, ab_h_chain, ab_l_chain, _, seqs, pdb in load_chains(summary_file):
        ...
        res = process_chains(
            ag_search, ab_h_chain, ab_l_chain, seqs, pdb, max_cdr_len=MAX_CDR_LEN
        )
        if res is None:
            continue
...
def get_cdrs_and_contact_info(ag_search, ab_h_chain, ab_l_chain, sequences, pdb):
    ...
    for cdr_name, cdr_chain in cdrs.items():
        contact[cdr_name] = [
            False if res[1] is None else residue_in_contact_with(res[1], ag_search)
    ...
    if num_in_contact < 5:
        print(
            "Antibody has very few contact residues: ", num_in_contact, file=sys.stderr
        )
        return None
...
def process_chains(ag_search, ab_h_chain, ab_l_chain, sequences, pdb, max_cdr_len):
    results = get_cdrs_and_contact_info(
        ag_search, ab_h_chain, ab_l_chain, sequences, pdb
    )
...
def download_annotated_sequences(
    dataset_desc_filename, cache_file="parapred/precomputed/downloaded_seqs.p"
):
    df = pd.read_csv(dataset_desc_filename)
