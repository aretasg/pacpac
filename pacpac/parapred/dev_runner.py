...
def single_run(dataset_file):
    ...
    history = model.fit(
        [cdrs_train, np.squeeze(masks_train)],
        lbls_train,
        batch_size=32,
        epochs=150,
        # Just a trial, not actual evaluation.
        validation_data=(
            [cdrs_test, np.squeeze(masks_test)],
            lbls_test,
            test_ex_weight,
        ),
...
def run_cv(dataset, output_folder, num_iters=10):
    ...
    for i in iters:
        ...
        kfold_cv_eval(model_factory, dataset, output_file, weights_template, seed=i)
...
def process_cv_results(cv_result_folder, abip_result_folder, cv_num_iters=10):
    ...
    fig = plot_pr_curve(labels, probs, colours=("#0072CF", "#68ACE5"), label="Parapred")
    fig = plot_pr_curve(
        labels_abip,
        probs_abip,
        colours=("#D6083B", "#EB99A9"),
        label="Parapred using ABiP data",
        plot_fig=fig,
    )
...
def patchdock_prepare():
    ...
    for name, probs in [("contact", contact), ("CDR", cdrs), ("parapred", parapred)]:
        ...
        annotate_and_save_test_structures(
            "parapred/data/dock_test.csv", probs, "annotated/" + name
        )
...
def patchdock_classify():
    ...
    print(
        capri_evaluate_test_structures("parapred/data/dock_test.csv", "results/contact")
    )

    print("Parapred results")
    print(
        capri_evaluate_test_structures(
            "parapred/data/dock_test.csv", "results/parapred"
        )
    )
